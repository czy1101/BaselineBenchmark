...
def _nsa_topk_paper_varlen_kernel(
    seq_num: int,
    c_seq_len: int,
    heads: int,
    dim: int,
    ...
    def _nsa_topk_paper_varlen_func(threads: int):
        ...
        @T.prim_func
        def _parallel_nsa_topk_paper_varlen_main(
        ...
        return _parallel_nsa_topk_paper_varlen_main

    return _nsa_topk_paper_varlen_func


@torch.library.custom_op("top::nsa_topk_paper_varlen_wrapped_kernel", mutates_args=())
def _nsa_topk_paper_varlen_wrapped_kernel(
    seq_num: int,
    c_seq_len: int,
    heads: int,
    dim: int,
    ...
    return _nsa_topk_paper_varlen_kernel(seq_num, c_seq_len, heads, dim, chunk_num, group, scale,
                                   selected_block_num, bc, bs, dtype,
                                   accum_dtype)(threads)(q, k_cmp, lse_in, offsets, chunk_offsets,
                                                         token_indices)


@_nsa_topk_paper_varlen_wrapped_kernel.register_fake
def _(
    seq_num: int,
    c_seq_len: int,
    heads: int,
...
class NSATopkPaperVarlenKernel(Kernel):
    ...
    def forward(self, q: torch.Tensor, k_cmp: torch.Tensor, lse_in: torch.Tensor,
        ...
        return _nsa_topk_paper_varlen_wrapped_kernel(self.seq_num, self.c_seq_len, self.heads, self.dim,
